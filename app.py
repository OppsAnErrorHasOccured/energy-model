...
            "<div class='eq-title'>Acceleration rate and force</div>",
            unsafe_allow_html=True,
        )
        st.latex(r"a = \frac{v}{t_{acc}} \qquad F_{acc} = m\,a "
                 r"\qquad d_{acc} = \tfrac{1}{2}\,v\,t_{acc}")
        st.markdown(
            f"<div class='eq-body'>Constant acceleration from rest to operating "
            f"speed. The distance covered while accelerating is subtracted from the "
            f"cruise distance used in the drag term so no metre is counted twice."
            f"<br><b>a = {a:.3f} m/s² · F_acc = {F_accel:,.0f} N · "
            f"d_acc = {d_accel_each:,.1f} m per stop</b></div></div>",
            unsafe_allow_html=True,
        )
...
                 r"m_{pack} = \frac{E_{kWh}}{e_{grav}}")
        st.markdown(
            f"<div class='eq-body'>e_grav = 0.16 kWh/kg at cell level (0.576 MJ/kg). "
            f"The pack is sized to complete one full route on a single charge.<br>"
            f"<b>{bev_kwh:,.1f} kWh → {bev_mass_d:,.0f} kg of cells</b></div></div>",
            unsafe_allow_html=True,
        )
...
            "gap rather than only its rank. Cost, mass and CO₂ use the first form; "
            "efficiency uses the second; infrastructure readiness is a fixed expert "
            "score of 100 / 30 / 60.</div></div>",
            unsafe_allow_html=True,
        )
    with f2:
        st.markdown(
...
            <b>Storage system mass.</b> Only the fuel or cells are weighed. Composite
            700-bar hydrogen tanks and battery enclosures, cooling and structure can
            more than double the figures shown, which understates the payload
            penalty for both zero-emission options.<br><br>
