...
# ==========================================================
C_DIESEL = "#8E8E93"  # System Gray (Apple)
C_HYDROGEN = "#0071E3"  # SF Blue (Apple)
C_BATTERY = "#34C759"  # SF Green (Apple)

# Apple-inspired Chart Palette
APPLE_PALETTE = [
    "#0071E3",
    "#34C759",
    "#AF52DE",
...
st.markdown(
    ...
    <style>
      ...
      .kv span.warn { color: #FF3B30; font-weight: 600; }
      .badge {
      ...
      .budget-fill.full { background: #34C759; }
...
with st.sidebar:
  ...
  with st.expander("Advanced settings"):
    ...
    h2_grav = st.slider(
        "H₂ tank system capacity (% H₂ by mass)",
        2.0,
        10.0,
        5.7,
        step=0.1,
        help=(
            "Fraction of the filled 700-bar tank system that is hydrogen. "
            "5.7% means the tank weighs about 17× the fuel it holds."
        ),
    )
    pack_factor = st.slider(
        "Battery pack integration factor",
        0.50,
        1.00,
        0.70,
        step=0.01,
        help=(
            "Cell-to-pack ratio. Enclosure, cooling, BMS and structure mean a "
    ...
    dod = st.slider(
        ...
        step=5,
        help=(
            "Packs are oversized so the route only draws this share of "
            "nameplate capacity."
        ),
    )
  ...
  for key, (label, _default) in WEIGHTS.items():
    ...
    st.slider(
        ...
        args=(key,),
        help=(
            f"Ceiling is {headroom}% while the other four hold their values. "
            "Anything higher snaps back to the remaining budget."
        ),
    )
...
rated_capacity_kg = max(
    (gvwr - curb_weight) * LB_TO_KG, 0.0
)  # payload if energy were weightless
...
with tab1:
  ...
  # ---------- Section: Score Origin & Tractive Breakdown (MOVED BEFORE SCORECARDS) ----------
  ...
  with left:
    ...
    for j, col in enumerate(contrib.columns):
      fig_c.add_bar(
          ...
          y=contrib[col],
          name=col,
          marker_color=APPLE_PALETTE[j % len(APPLE_PALETTE)],
  ...
  fig_p.add_hline(
      y=required_payload,
      line_dash="dash",
      line_color="#FF3B30",  # Apple SF Red
  ...
  fig_p.add_vline(
      x=route_dist,
      line_dash="dot",
      line_color="#8E8E93",  # Apple System Gray
  ...
  fig_p.update_layout(
      ...
      font=dict(
          family="-apple-system, BlinkMacSystemFont, Inter, sans-serif", size=12
      ),
...
with tab2:
  ...
  # ---------- Stage 1 ----------
  st.markdown(
      "<div class='section-title' style='margin-top:1.6rem;'>Stage 1 — Road"
      " load</div><div class='section-note'>Three resistances consume energy"
      " on an urban refuse route. Grade is neglected: the modeled route returns"
      " to its starting elevation, so net potential energy is zero.</div>",
      unsafe_allow_html=True,
  )
  ...
  with c1:
    ...
    st.markdown(
        ...
        f"<b>C_rr = {crr:.3f} · F_rr = {F_rr:,.0f} N · E_rr = {E_rr:,.2f}"
        " MJ/day</b></div></div>",
        unsafe_allow_html=True,
    )
  ...
  with c2:
    ...
    st.markdown(
        ...
        f"F_acc = {F_accel:,.0f} N · d_acc = {d_accel_each:,.1f} m per"
        " stop</b></div></div>",
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        ...
        f"regeneration every joule here is thrown away as brake heat at the"
        " next "
        f"stop — the defining penalty of refuse duty.<br>"
        f"<b>{n_stops} stops × {0.5*m*v**2/1e6:,.3f} MJ = {E_accel:,.2f}"
        " MJ/day</b></div></div>",
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        f"<div class='eq-body'>Cabin heating, hydraulics and the compaction"
        " body "
  ...
  # ---------- Stage 2 ----------
  st.markdown(
      "<div class='section-title' style='margin-top:1.4rem;'>Stage 2 —"
      " Powertrain efficiency</div><div class='section-note'>The same wheel"
      " energy costs three very different amounts of stored energy.</div>",
      unsafe_allow_html=True,
  )
  st.markdown(
  ...
  st.markdown(
      ...
      f"transmission and driveline losses. Diesel loses roughly three quarters"
      " of "
  ...
  # ---------- Stage 3 ----------
  st.markdown(
      "<div class='section-title' style='margin-top:1.4rem;'>Stage 3 — Fuel,"
      " cost, mass, emissions</div><div class='section-note'>Energy is"
      " converted into the unit each fuel is actually sold and stored"
      " in.</div>",
      unsafe_allow_html=True,
  )
  ...
  with d2:
    st.markdown(
        "<div class='eq-card'><div class='eq-num'>Equation 09 ·"
        " Hydrogen</div><div class='eq-title'>Mass of hydrogen</div>",
        unsafe_allow_html=True,
    )
  ...
  with d3:
    ...
    st.latex(
        r"E_{kWh} = \frac{E_{source}}{3.6} \qquad E_{nameplate} ="
        r" \frac{E_{kWh}}{DoD}"
    )
    st.markdown(
        ...
        f"<b>{bev_kwh:,.1f} kWh drawn → {bev_nameplate_kwh:,.1f} kWh"
        " installed</b></div></div>",
        unsafe_allow_html=True,
    )
  ...
  with p1:
    ...
    st.latex(
        r"m_{sys}^{BEV} = \frac{E_{nameplate}}{e_{grav}\cdot \kappa_{pack}}"
    )
    st.markdown(
        ...
        f"<b>γ = {h2_grav:.1f}%</b> hydrogen by mass, so the tank outweighs"
        " its "
        f"contents roughly {100/h2_grav:.0f}-fold — the single biggest"
        " correction "
        f"to the original workbook. Cells become a pack through the"
        " integration "
  ...
  with p2:
    ...
    st.markdown(
        ...
        f"weight allowance under the GVWR. Because <b>m_sys</b> grows with"
        " route "
  ...
  with e1:
    ...
    st.latex(
        r"C_{daily} = Q_{fuel}\cdot p_{fuel} \qquad C_{annual} ="
        r" C_{daily}\cdot N_{days}"
    )
    st.markdown(
        f"<div class='eq-body'>Q is gallons, kilograms or kilowatt-hours "
        f"depending on the technology. This is an energy cost only — no"
        " capital, "
  ...
  # ---------- Stage 4 ----------
  st.markdown(
      "<div class='section-title' style='margin-top:1.4rem;'>Stage 4 — Decision"
      " logic</div><div class='section-note'>Five criteria in different units"
      " are reduced to a common 0–100 scale, then weighted.</div>",
      unsafe_allow_html=True,
  )
  ...
  with f1:
    ...
    st.latex(
        r"S_i = \frac{x_{\min}}{x_i}\times 100 \quad\text{(lower is better)}"
    )
    st.latex(
        r"S_i = \frac{x_i}{x_{\max}}\times 100 \quad\text{(higher is better)}"
    )
    st.markdown(
        "<div class='eq-body'>Ratio scaling is used instead of min–max scaling"
        " so the worst option never scores a hard zero. A technology twice as"
        ...
        " efficiency and usable payload use the second; infrastructure"
        " readiness is a fixed expert score of 100 / 30 / 60. Note that cost"
        " and CO₂ ratios are scale-invariant — every powertrain's consumption"
        " is proportional to the same tractive energy, so route length cancels"
        " out of those two criteria. Payload and regeneration are the terms"
        " that carry duty-cycle sensitivity.</div></div>",
        unsafe_allow_html=True,
    )
  with f2:
    st.markdown(
    ...
    st.latex(
        r"\text{Score} = \frac{\sum_{i=1}^{5} w_i S_i}{\sum_{i=1}^{5} w_i}"
        r"\qquad \sum_{i=1}^{5} w_i \le 100"
    )
    st.markdown(
        f"<div class='eq-body'>The five weights are hard-capped so they can"
        f" never exceed a 100% budget; each slider's ceiling shrinks as the"
        f" others are raised. Dividing by the allocated total keeps the result"
        f" on a 0–100 scale even when part of the budget is left unspent.<br>"
